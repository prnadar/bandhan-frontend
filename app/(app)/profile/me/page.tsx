    ...
    <div className="px-8 py-8 max-w-4xl">
      ...
      <div className="grid md:grid-cols-[280px_1fr] gap-6">
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <div className="h-2 bg-deep/8 rounded-full overflow-hidden">
              <div className="h-full rounded-full" style={{ width: `${completeness}%`, background: "linear-gradient(90deg,#C4520F,#C89020)" }} />
            </div>
        ...
        <div className="space-y-4">
          ...
          {/* About */}
          <ProfileSection title="About Me" icon={<Heart className="w-4 h-4 text-marigold" />}>
          ...
          {/* Hobbies */}
          <ProfileSection title="Hobbies & Interests" icon={<Star className="w-4 h-4 text-marigold" />}>
          ...
          {/* Partner Preferences */}
          <ProfileSection title="Partner Preferences" icon={<Heart className="w-4 h-4 text-marigold" />}>
          ...
          >
            <Shield className="w-6 h-6 text-marigold flex-shrink-0 mt-0.5" />
...
function EditableText({ field, value, editing, setEditing, onSave, multiline }: {
      ...
      <div className="flex items-end gap-2">
        {multiline
          ? <textarea
              ...
              style={{ background: "rgba(255,255,255,0.8)", border: "1.5px solid #C4520F" }}
            />
          : <input
              autoFocus
              value={draft}
        ...
        <div className="flex gap-1">
          <button onClick={() => onSave(field, draft)} className="px-3 py-1.5 rounded-lg font-body text-xs font-semibold text-white" style={{ background: "linear-gradient(135deg,#C4520F,#E06A1A)", minHeight: "auto" }}>Save</button>
  ...
  return (
    <div
      className="group flex items-start justify-between gap-2 cursor-pointer rounded-lg px-2 py-1 -mx-2 hover:bg-marigold/5 transition-colors"
      onClick={() => { setDraft(value); setEditing(field); }}
    >
      <span className="font-body text-sm text-deep/75 leading-relaxed flex-1">{value}</span>
      <Edit3 className="w-3.5 h-3.5 text-deep/25 group-hover:text-marigold flex-shrink-0 mt-0.5 transition-colors" />
    </div>
  );
}
